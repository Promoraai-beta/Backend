 ...
 * In production, all logs are silently ignored to reduce noise and improve performance.
 * 
 * Usage:
...
const forceLoggingEnabled = process.env.ENABLE_LOGGING === 'true';
...
export const logger = {
   ...
   * But you can disable them by setting ENABLE_LOGGING=false
   */
  error: (...args: any[]) => {
    if (isLoggingEnabled) {
      console.error(...args);
